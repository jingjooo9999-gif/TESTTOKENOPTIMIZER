...
function appendRequestRow(record, prepend = true) {
  ...
  tr.className = 'hover:bg-slate-800/40 transition group border-b border-slate-800/40 text-slate-300';

  // Clean status without pill capsules (pure text + icon)
  const statusText = record.cached
    ? `<span class="inline-flex items-center gap-1.5 text-xs font-medium text-cyan-400"><i data-lucide="zap" class="w-3.5 h-3.5 text-cyan-400"></i> Cached (0ms)</span>`
    : `<span class="inline-flex items-center gap-1.5 text-xs font-medium text-emerald-400"><i data-lucide="check" class="w-3.5 h-3.5 text-emerald-400"></i> Optimized</span>`;

  tr.innerHTML = `
    <td class="py-3.5 px-4 text-slate-400 text-[11px] whitespace-nowrap">${record.timestamp}</td>
    <td class="py-3.5 px-4 whitespace-nowrap">
      <div class="font-semibold text-slate-200 text-[11px]">${record.model}</div>
      <div class="text-[10px] text-slate-500 uppercase font-sans">${record.provider}</div>
    </td>
    <td class="py-3.5 px-4 max-w-xs truncate text-slate-300 font-sans text-[11px]" title="${record.promptPreview}">
      ${record.promptPreview}
    </td>
    <td class="py-3.5 px-4 text-right text-slate-400 font-mono">${formatNumber(record.originalTokens)}</td>
    <td class="py-3.5 px-4 text-right text-slate-300 font-medium font-mono">${formatNumber(record.optimizedTokens)}</td>
    <td class="py-3.5 px-4 text-right font-mono">
      <span class="text-emerald-400 font-bold">-${record.percentageSaved}%</span>
      <div class="text-[10px] text-slate-500">(${formatNumber(record.savedTokens)} tok)</div>
    </td>
    <td class="py-3.5 px-4 text-right font-bold text-emerald-400 font-mono">+$${record.dollarsSaved.toFixed(4)}</td>
    <td class="py-3.5 px-4 text-center whitespace-nowrap">${statusText}</td>
  `;
  ...
  eventSource.onmessage = (event) => {
    try {
      ...
      if (data.type === 'INIT') {
        ...
        requestTableBody.innerHTML = `
          <tr id="empty-row">
            <td colspan="8" class="py-10 text-center text-slate-500 font-sans">
              <div class="flex flex-col items-center justify-center gap-2">
                <i data-lucide="inbox" class="w-8 h-8 text-slate-600"></i>
                <span>Stats reset. Listening for new requests...</span>
              </div>
            </td>
          </tr>
        `;
...
if (toggleLogCleaner) toggleLogCleaner.addEventListener('change', updateSettings);
if (togglePromptMinifier) togglePromptMinifier.addEventListener('change', updateSettings);
if (toggleSmartCache) toggleSmartCache.addEventListener('change', updateSettings);
...
if (btnTriggerSim) {
  btnTriggerSim.addEventListener('click', async () => {
    ...
    btnTriggerSim.innerHTML = '<span class="animate-spin inline-block mr-1">⚙️</span> Simulating...';
...
Please provide a fix for line 42.`
    ...
    try {
      ...
      btnTriggerSim.innerHTML = '<i data-lucide="zap" class="w-4 h-4 inline mr-1"></i> Simulate Request';
      refreshIcons();
    }
  });
}
...
if (btnTriggerMoE) {
  btnTriggerMoE.addEventListener('click', async () => {
    ...
    btnTriggerMoE.innerHTML = '<span class="animate-spin inline-block mr-1">⚙️</span> Streaming SSD...';
    ...
    try {
      ...
      if (moeLiveTrace) {
        moeLiveTrace.innerHTML = '<span class="text-emerald-400 font-medium">⚡ Streamed 8 tokens via SSD bus (0ms $0)</span>';
      }
    } catch (err) {
      if (moeLiveTrace) moeLiveTrace.innerText = 'Stream error: ' + err.message;
    } finally {
      btnTriggerMoE.disabled = false;
      btnTriggerMoE.innerHTML = '<i data-lucide="play" class="w-4 h-4 inline mr-1 fill-white"></i> Run Local MoE';
      refreshIcons();
    }
  });
}
